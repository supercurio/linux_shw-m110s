...
void Ap_Cp_Switch_Config(u16 ap_cp_mode)
{
	switch (ap_cp_mode) {
		...
		case AP_UART_MODE:
            ...
            if (HWREV==0)
    			gpio_set_value(GPIO_UART_SEL_R00, 1);
            else
                gpio_set_value(GPIO_UART_SEL_R01, 1);
#endif
			break;
		case CP_USB_MODE:
			usb_path=2;
...
#elif defined CONFIG_M115S
            if (HWREV==0)
    			gpio_set_value(GPIO_UART_SEL_R00, 0);
            else
                gpio_set_value(GPIO_UART_SEL_R01, 0);
#endif
			break;
		default:
...
)
{
	...
	if(value != usb_access_lock) {
		...
		if(value == 1) {
			printk("[USB_LOCK] Set USB Block!!\n");
			android_usb_set_connected(0);
			UsbIndicator(0);
			s3c_usb_cable(0);
		
		} else {
			printk("[USB_LOCK] Release USB Block!!\n");
...
void UsbIndicator(u8 state)
{
	...
	if(!state && !indicator_dev.state) {
		printk(KERN_DEBUG "force send usb uevent\n");
...
static DEVICE_ATTR(ums, S_IRUGO |S_IWUGO | S_IRUSR | S_IWUSR, ums_switch_show, ums_switch_store);
...
{
#ifndef USB_LATONA_FEATURE
	DEBUG_FSA9480("[FSA9480] %s (dev1 : 0x%x, dev2 : 0x%x)\n", __func__, dev1, dev2);
#else
	printk(" <d1:0x%x,d2:0x%x,a:0x%x> ", dev1, dev2, attach);
#endif
...
bool wait_cp_boot_ok(int lpm_mode)
{
	static unsigned int switching_init_timeout = 100;

	if (switching_init_timeout <= 0 
		|| lpm_mode 
		|| factoryresetstatus == 0xAE
		|| cp_boot_ok)
		return false;

	//printk(KERN_DEBUG"[FSA9480]wait_cp_boot_ok=%d [%d]\n", cp_boot_ok, switching_init_timeout);
	switching_init_timeout--;
	return true;
}
...
void connectivity_switching_init(struct work_struct *ignored)
{
	...
	if (sec_get_param_value && wait_cp_boot_ok(lpm_mode_check) == false){
		sec_get_param_value(__SWITCH_SEL, &switch_sel);
	...
	printk(KERN_DEBUG "[FSA9480]connectivity_switching_init = switch_sel : 0x%x cp_boot_ok=%d\n",switch_sel, cp_boot_ok);
...
static ssize_t print_switch_state(struct switch_dev *sdev, char *buf)
{
	...
	if (usb_access_lock)
	{
		printk(KERN_DEBUG "RemoveOffline : usb_lock\n");
		return sprintf(buf, "%s\n", "RemoveOffline");
	}
#endif
...
//TODO : each platform require different noti
    ...
    if(usbstatus){
        ...
        else if((currentusbstatus== USBSTATUS_UMS) || (currentusbstatus== USBSTATUS_ADB) || (currentusbstatus== USBSTATUS_SAMSUNG_KIES))
        {
        ...
        if((currentusbstatus== USBSTATUS_UMS) || (currentusbstatus== USBSTATUS_ADB) || (currentusbstatus== USBSTATUS_SAMSUNG_KIES))
        {
...
static int fsa9480_codec_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (HWREV==0)
    {
    	s3c_gpio_cfgpin(GPIO_UART_SEL_R00, S3C_GPIO_OUTPUT );
    	s3c_gpio_setpull(GPIO_UART_SEL_R00, S3C_GPIO_PULL_NONE);
    }
    else
    {
    	s3c_gpio_cfgpin(GPIO_UART_SEL_R01, S3C_GPIO_OUTPUT );
    	s3c_gpio_setpull(GPIO_UART_SEL_R01, S3C_GPIO_PULL_NONE);
    }
#endif
	...
	if (device_create_file(switch_dev, &dev_attr_ums) < 0)		
