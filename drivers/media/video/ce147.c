...
struct ce147_state {
	...
	int thumb_null;//ykh
#endif
#if 0	//[ block aeawb lock operation : because of touch AF concept ]	def CONFIG_M115S
	int disable_aeawb_lock;		//aeawb lock
#endif
};
...
#endif
#ifdef CONFIG_M115S
	unsigned char ce147_str_model[10] = "SHW-M115S\0";
#endif
...
#ifdef CONFIG_M115S //Nightshot/BeachSnow/Firework delay of LSI Sensor
	if((state->iso == ISO_NIGHT) || (state->iso == ISO_50))
	{
		/*
...
#if 0	//[ block aeawb lock operation : because of touch AF concept ]	def CONFIG_M115S		//aeawb lock
	err = ce147_set_awb_lock(sd, 0);
	if(err < 0){
		dev_err(&client->dev, "%s: failed: ce147_set_awb_lock, err %d\n", __func__, err);
		return -EIO;
...
#if 0	//[ block aeawb lock operation : because of touch AF concept ]	def CONFIG_M115S
	struct ce147_state *state = to_state(sd);		//aeawb lock
#endif
