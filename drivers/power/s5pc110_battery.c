        ...
        switch (off)
    {
        ...
        case BATT_TEMP_ADC_SPEC:
            i += scnprintf(buf + i, PAGE_SIZE - i, "(HIGH: %d - %d,   LOW: %d - %d)\n", 
            			(4096 - TEMP_HIGH_BLOCK)/10,
            			(4096 - TEMP_HIGH_RECOVER)/10,
            			(4096 - TEMP_LOW_BLOCK)/10,
            			(4096 - TEMP_LOW_RECOVER)/10);
            break;
#ifdef __TEST_MODE_INTERFACE__
        case BATT_VOL_AVER:
...
static int s3c_bat_get_charging_status(void)
{
    ...
    //pr_info("[BAT]:%s\n", __func__);
        ...
        if(ibatt_test_value == 777 || ibatt_test_value == 999)
        {
...
static unsigned long s3c_read_temp(void)
{
	...
	if(adc == 0) /* abnormal case */
	{
		adc = s3c_bat_info.bat_info.batt_temp_adc;
	}
...
static void s3c_temp_control(int mode)
{
    ...
    //if(except_temp_block != 0)
    {
		...
		if((mode == POWER_SUPPLY_HEALTH_OVERHEAT) && (s3c_bat_info.bat_info.batt_temp_adc >= TEMP_EVENT_HIGH_BLOCK)) {
#else /* M110S */
        if(mode != POWER_SUPPLY_HEALTH_GOOD) {
...
static void s3c_cable_check_status(void)
{
    ...
    else
    {
        ...
        s3c_bat_info.bat_info.batt_is_recharging = 0; /* s1-kor feature */
		/* fix always 100% problem */
		s3c_bat_info.bat_info.batt_is_full = 0; /* s1-kor feature */
...
static int s3c_get_bat_level(void)
{
	...
	if(!s3c_bat_info.bat_info.charging_enabled && ((s3c_bat_info.bat_info.batt_vol > 2800) && (s3c_bat_info.bat_info.batt_vol < 3350))) {
		low_volt_count++;
		if((low_volt_count > 4) && (fg_soc != 0)) {
			printk("low voltage, set fg_soc to 0\n");
			low_volt_count = 5;
			fg_soc = 0;
		}
	}
...
static int __devinit s3c_bat_probe(struct platform_device *pdev)
{
    ...
    // Here, check again.
    mutex_lock(&work_lock);
    if((fg_read_rcomp()&0xff1f)!=0xff1f) {
        fuel_gauge_rcomp();
        fg_read_rcomp();
    }
    mutex_unlock(&work_lock);
