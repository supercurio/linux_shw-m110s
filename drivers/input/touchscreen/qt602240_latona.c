...
static u8 is_suspending = 0; // sooo.shin touchkey_led
...
//					cal_check_flag = 2u;
				...
				if ((quantum_msg[0] == 0xff) && (quantum_msg[1] == 0xff) && (quantum_msg[2] == 0xff)) {
					check_chip_calibration(1);
				}				
...
static int touchkey_control(int data)
{
	...
	if (gpio_is_valid(GPIO_LED_EN1)) 
	{
			if (gpio_request(GPIO_LED_EN1, "GPJ3"))
					printk("Failed to request GPIO_LED1_EN!\n");
			
			s3c_gpio_cfgpin(GPIO_LED_EN1, S3C_GPIO_OUTPUT);
			gpio_direction_output(GPIO_LED_EN1, (int)data);
	}
	s3c_gpio_setpull(GPIO_LED_EN1, S3C_GPIO_PULL_NONE);
	gpio_free(GPIO_LED_EN1);
	
	if (gpio_is_valid(GPIO_LED_EN2)) 
	{
			if (gpio_request(GPIO_LED_EN2, "GPJ3"))
					printk("Failed to request GPIO_LED1_EN!\n");
			
			s3c_gpio_cfgpin(GPIO_LED_EN2, S3C_GPIO_OUTPUT);
			gpio_direction_output(GPIO_LED_EN2, (int)data);
	}
	s3c_gpio_setpull(GPIO_LED_EN2, S3C_GPIO_PULL_NONE);
	gpio_free(GPIO_LED_EN2);
...
static ssize_t set_refer_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	status = read_dbg_data(QT_REFERENCE_MODE, test_node,&qt_refrence);
	return sprintf(buf, "%u\n", qt_refrence);	
}
...
static ssize_t set_refer_mode_store(struct device *dev, struct device_attribute *attr,	const char *buf, size_t size)
{
	...
	test_node = simple_strtoul(buf, &after, 10);
...
static ssize_t set_delta_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	status = read_dbg_data(QT_DELTA_MODE, test_node,&qt_delta);
	if(qt_delta < 32767){
		return sprintf(buf, "%u\n", qt_delta);	
	   }
	else	{
...
static DEVICE_ATTR(set_refer, S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH, set_refer_mode_show, set_refer_mode_store);
static DEVICE_ATTR(set_all_refer, S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH, set_all_refer_mode_show, NULL);
static DEVICE_ATTR(set_delta, S_IRUGO | S_IWUSR | S_IWOTH | S_IXOTH, set_delta_mode_show, set_delta_mode_store);
