...
static int s3cfb_release_window(struct fb_info *fb)
{
	...
	struct s3c_platform_fb *pdata = to_fb_plat(fbdev->dev);
	struct s3cfb_window *win = fb->par;
	// Assume that first fb release is done by logo animation.
	if (win->id == pdata->default_win) {
		if (first_release == true)
		{
			dev_info(fbdev->dev, "clear default fb\n");
			memset(fb->screen_base, 0, fb->fix.smem_len);	
			first_release = false;
		}
	}
...
void s3cfb_early_suspend(struct early_suspend *h)
{
	...
	printk(KERN_DEBUG "s3cfb_early_suspend is called\n");
...
void s3cfb_late_resume(struct early_suspend *h)
{
	...
	printk(KERN_DEBUG "s3cfb_late_resume is called\n");
