...
#define CRITICAL_BATTERY_LEVEL 10
...
static bool need_bl_update = false;
static int requested_gamma_value = 0;
...
void Lcd_cabc_on(void)
{
	...
	if (cur_cabc_mode != cabc_mode)
	{
		...
		{
			case CABC_UI_MODE:
				s6e63m0_panel_send_sequence(nt35510_SEQ_CABC_UI);
				break;
			case CABC_VIDEO_MODE:
				s6e63m0_panel_send_sequence(nt35510_SEQ_CABC_VIDEO);
				break;
			case CABC_IMAGE_MODE:
				s6e63m0_panel_send_sequence(nt35510_SEQ_CABC_IMAGE);
				break;
			default:
				s6e63m0_panel_send_sequence(nt35510_SEQ_CABC_UI);
				break;
...
void Lcd_cabc_off(void)
{
	...
	{
		printk(KERN_DEBUG "update cabc mode = %d\n", cur_cabc_mode); 
		s6e63m0_panel_send_sequence(nt35510_SEQ_CABC_OFF);
        ...
        struct device_attribute *attr, const char *buf, size_t size)
{
	...
	{
		case 1:
			s6e63m0_panel_send_sequence(nt35510_SEQ_CABC_UI);	cur_cabc_mode = CABC_UI_MODE;	printk("[%s] set CABC_UI\n", __func__);
			break;
		case 2:
			s6e63m0_panel_send_sequence(nt35510_SEQ_CABC_IMAGE); cur_cabc_mode = CABC_IMAGE_MODE;	printk("[%s] set CABC_IMAGE\n", __func__);
			break;
		case 3:
			s6e63m0_panel_send_sequence(nt35510_SEQ_CABC_VIDEO); cur_cabc_mode = CABC_VIDEO_MODE;	printk("[%s] set CABC_VIDEO\n", __func__);
		default:
			s6e63m0_panel_send_sequence(nt35510_SEQ_CABC_OFF); cur_cabc_mode = CABC_OFF_MODE;	printk("[%s] set CABC_OFF\n", __func__);
			break;
	}
...
static DEVICE_ATTR(cabc_mode,0666, cabc_mode_show, cabc_mode_store);
...
static int tl2796_check_LcdPanel(void)
{
	...
	if(lcd_id1 ==0 && lcd_id2 != 0)
		g_lcd_type = 1; // Sony Panel
	else
		g_lcd_type = 0; // hitachi Panel

	printk(KERN_DEBUG "lcd_type = %x (%d %d)\n", g_lcd_type, lcd_id1, lcd_id2);
...
void tl2796_ldi_init(void)
{
	...
	if (g_lcd_type == 1)
	{
		s6e63m0_panel_send_sequence(sony_SEQ_SETTING);
	s6e63m0_panel_send_sequence(s6e63m0_SEQ_DISPLAY_ON);	
	}
	else
	{
		s6e63m0_panel_send_sequence(hitachi_SEQ_SETTING);
		s6e63m0_panel_send_sequence(s6e63m0_SEQ_DISPLAY_ON);
	}	
#else
...
{
	if (need_bl_update && current_gamma_value == -1)
	{
		printk(KERN_DEBUG "update brightness with %d level\n", requested_gamma_value);
		update_brightness(requested_gamma_value);
	}
}
...
void tl2796_ldi_disable(void)
{
	...
	s6e63m0_panel_send_sequence(nt35510_SEQ_SLEEP_IN);
#else
...
void s3cfb_set_lcd_info(struct s3cfb_global *ctrl)
{
	...
	if(g_lcd_type == 0)	// Hitachi panel
	{
		s6e63m0.timing.h_fp = 100;
		s6e63m0.timing.h_bp = 2;
		s6e63m0.timing.h_sw = 2;
		s6e63m0.timing.v_fp = 8;
		s6e63m0.timing.v_bp = 10;
		s6e63m0.timing.v_sw = 2;

		s6e63m0.polarity.rise_vclk = 1;
		s6e63m0.polarity.inv_hsync = 1;
		s6e63m0.polarity.inv_vsync = 1;
		s6e63m0.polarity.inv_vden = 0;
	}
#endif
	ctrl->lcd = &s6e63m0;
}
...
static int s5p_bl_update_status(struct backlight_device* bd)
{
	...
	if (!IsLDIEnabled())
	{
		printk(KERN_DEBUG "skip update brightness : ldi is off \n");
#if defined(CONFIG_M115S)
		requested_gamma_value = get_pwm_value_from_bl(bl);
#else
		requested_gamma_value = get_gamma_value_from_bl(bl);
#endif
		need_bl_update = true;
		return 0;
	}
	...
	printk(KERN_DEBUG "bl=%d  g=%d l=%d \n", bl, gamma_value, level);	
...
#if defined(CONFIG_M115S)
	if (device_create_file(switch_aclset_dev, &dev_attr_cabc_mode) < 0)
