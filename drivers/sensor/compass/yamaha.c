...
#define YAMAHA_GSENSOR_TRANSFORMATION_S1T \
    { { -1,  0, 0}, \
      { 0,  -1,   0}, \
      { 0 , 0 ,  -1} }
...
#define YAMAHA_MSENSOR_TRANSFORMATION_S1T    \
    { { -1,  0,  0}, \
      { 0,  1,  0}, \
      { 0,  0,  -1} }
	      ...
	      unsigned int cmd, unsigned long arg)
{
	...
	signed char garray_S1T[3][3] = YAMAHA_GSENSOR_TRANSFORMATION_S1T;
	signed char marray_S1T[3][3] = YAMAHA_MSENSOR_TRANSFORMATION_S1T;
	...
	switch (cmd) {
		case YAMAHA_IOCTL_GET_MARRAY:
			...
			if (copy_to_user((void *)arg, marray_S1T, sizeof(marray_S1T))) 
				{
					printk("YAMAHA_MSENSOR_TRANSFORMATION_S1T copy failed\n");
					return -EFAULT;
				}
				printk(KERN_DEBUG "YAMAHA_MSENSOR_TRANSFORMATION_S1T copy OK\n");
#endif
			break;
...
#elif defined CONFIG_M115S
			if (copy_to_user((void *)arg, garray_S1T, sizeof(garray_S1T))) 
				{
					printk("YAMAHA_GSENSOR_TRANSFORMATION_S1T copy failed\n");
					return -EFAULT;
				}
#endif
				printk("YAMAHA_GSENSOR_TRANSFORMATION_S1T copy OK\n");
